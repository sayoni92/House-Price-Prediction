kc_house_data <- read.csv("C:/Users/chatt/OneDrive/Desktop/Advanced Statistics/Project/kc_house_data.csv")
View(kc_house_data)
...
set.seed(9876)
index <- sample(2,nrow(HouseData),replace = T,prob = c(0.7,0.3))
...
HouseData_rpart <- rpart(price ~ bedrooms + bathrooms + sqft + sqft_lot 
                         ...
                         ,data=trainData,parms = list(split="gini"),control = rpart.control(minbucket = 10,cp=-0.01))
...
model_rf <- randomForest(tree_rf, data=HouseData, ntree=100,nodesize=10,proximity=T)# change ntree 
...
library(e1071)
model_svr <- svm(price ~ grade+bathrooms+sqft_living15,data = trainData,kernel="linear")
summary(model_svr)

test_pred <- predict(model_svr, newdata = testData[,-1])
pred_err_test <- testData$price - test_pred
pred_sqerr_test <- pred_err_test^2
mse_test <- mean(pred_sqerr_test)
mse_test

model_svr <- svm(price ~ .,data = trainData,kernel="linear")
summary(model_svr)
...
set.seed(825)
i <- sample(2, nrow(HouseData), replace=TRUE, prob=c(0.7, 0.3))
trainData <- HouseData[i==1,]
testData <- HouseData[i==2,]
nrow(trainData)
nrow(testData)
...
model_gbm <- train(tree_gbm, data = trainData, method = "gbm", trControl = fitControl, verbose = FALSE1)
print(model_gbm)
